...
public class FileIndexTester extends AbstractTester {
	...
	public static void testFileIndex(){
		...
		try {
			for (String word:fIndex.rankedWords.keySet()) {
				System.out.println(word + " , " + fIndex.getCountInFile("rocky1.txt", word) + " , "
						+ fIndex.getCountInFile("rocky2.txt", word) + " , " + 
						fIndex.getCountInFile("rocky3.txt", word));
			}
